...
async function generateKeyPair() {
  try {
    ...
    const pubRaw = await window.crypto.subtle.exportKey(
      "spki",
      keyPair.publicKey,
    );
    const pubB64 = b64encode(pubRaw);
    generatedPublicPEM = `-----BEGIN PUBLIC KEY-----\n${pubB64.match(/.{1,64}/g).join("\n")}\n-----END PUBLIC KEY-----`;
    generatedPrivateRaw = await window.crypto.subtle.exportKey(
      "pkcs8",
      keyPair.privateKey,
    );
...
document.querySelector("form").addEventListener("submit", async function (e) {
  ...
  try {
    const { wrappingKey, saltB64 } = await deriveWrappingKey(
      username,
      password,
    );
    const { wrapped, iv } = await wrapPrivateKey(
      generatedPrivateRaw,
      wrappingKey,
    );
